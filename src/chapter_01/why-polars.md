...
三笔账分开算。从 pandas 迁过来，真正的成本不在语法而在执行模型的心智：pandas 每一步都立即执行、随时可以打印中间结果排查问题，Polars 的链式表达则是先把计算描述完、collect 时才一次执行，排查方式从"看每一步的输出"变成"看计划与最终结果"。好在概念层两者几乎一一对应——select/filter/group_by/join 的语义可以直接平移——所以短脚本近乎机械改写，下一小节就是直观对照；真正要重训的只有"先写完再执行"这一条习惯，管道越长越值得先按第 6 章的方法梳理数据流再动手。

对 Spark 的账要算在分布式本身的固定开销上：数据在 executor 之间流动要经网络与序列化，每次 shuffle 都是一笔整表级的传输与编解码税，而单机引擎的数据始终驻留在同一块内存里，线程之间只传递数据分片的引用，这笔税根本不存在。运维上同样不对称：Spark 需要 JVM、集群管理与多组件版本协调，Polars 只是随脚本安装的一个 pip 依赖。对"改一版口径就重跑"的迭代式分析，每轮作业的提交与预热延迟会被反复放大，这正是两类工具体验差距的主要来源。
