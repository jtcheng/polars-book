...
两条路径的分野在数据何时进内存：read 在调用点就把整个文件解码成 DataFrame，之后的过滤与投影都作用在这份全量副本上，I/O 与内存都按文件大小记账；scan 只登记一个"将来读这个文件"的计划节点，真正的读取推迟到 collect——届时过滤条件与所需列已经确定，引擎可以只反序列化用到的列、能跳过的数据块直接跳过（行组级的跳过机制在 3.3 节展开）。第三条则是贯穿全书的骨架：读取端 scan、写出端 sink，两端都不物化全量，中间的变换以固定大小的块流过引擎，内存占用从"与数据总量同阶"降到"与块大小和聚合状态同阶"——这是流式主线（第 8 章）的入口，本章先把两端接起来。
...
sink 与 collect 的差别在管道的终点：collect 的终点是一张完整物化的表，sink 则把写出器直接接成计划的终点——每产出一批结果就落一批盘，任何时刻内存里只有正在写的那一块。与 scan 配对后两端都不经全量：读进来的按块过滤聚合、算出来的按块写走，端到端内存只与块大小和聚合状态相关，与数据总量无关。这也是它与"collect 完再 write"的本质区别——后者要先为整个结果付一次内存，大结果集当场触顶。
