...
# orders.parquet：沿用第 4 章生成的 12 列订单表（user_id/ts/amount + col_00~08）
# 逐步赋值：三次物化、三次全表遍历（一次磁盘读 + 两次内存遍历）
...
# 反模式 2：链中夹带 UDF —— UDF 之后的谓词无法下推到扫描层
(pl.scan_parquet("big.parquet")
   .with_columns(pl.col("x").map_elements(complex_fn))  # ❌ 断流
   .filter(pl.col("y") > 0))                            # y 的过滤本可下推，却被 UDF 挡住

# 正确：不依赖 UDF 输出的谓词放前面，把过滤下推到读取阶段
(pl.scan_parquet("big.parquet")
   .filter(pl.col("y") > 0)              # ✅ 可下推的谓词放前面
   .with_columns(pl.col("x").map_elements(complex_fn)))
# 注意：若谓词引用 UDF 的输出列（如 filter(pl.col("x") > 0) 且 x 已被 UDF 改写），
# 先后顺序不可交换——重排会改变语义。此时要么接受断流，要么把 UDF 改写成原生表达式。
```
