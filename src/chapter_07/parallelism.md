...
Polars 的 Rust 引擎自带线程池，调度交给 Rayon：任务分进各线程的本地队列，谁先做完就从别人队列尾部"偷"任务，于是行数不均的块、代价悬殊的算子被自动摊平到所有核，不需要你写一行调度代码。池子在 import 时按逻辑核数创建一次、此后不可变——想改大小只能留给下个进程（7.4 节展开）。默认值隐含"整机独占"的假设，容器配额或混部场景恰恰要警惕它。
...
按行分发会被调度的固定成本淹没：每派发一个任务都要入队、唤醒线程、归并结果，单行的计算量摊不平这笔开销；十万行量级的块既把成本摊到可忽略，又足够小，长尾任务能被别的线程偷走补齐。这个粒度同时是 I/O 友好的——一个 morsel 对应几 MB 连续的列数据，与操作系统按页缓存、按块预取的粒度同阶，扫描时进缓存的都是马上要用的整块，而不是零散碎片。
...
GIL 序列化的是 Python 字节码的执行，而构建链的调用只是轻量记账——往计划树里加一个节点，成本以微秒计；真正的计算发生在 collect 之后，Rust 引擎进入并行区前就放开了 GIL，这期间 Python 世界完全自由。语言切换一次查询只发生两次（进 Rust 一次、出 Python 一次），而不是每个算子一次，边界开销可以忽略。
...
池子大小在 import polars 的那一刻定死：初始化时读一次环境变量并创建线程池，此后没有运行时 API 能修改，所以设置必须赶到 import 之前，晚一步就只能留给下个进程。需要干预的动机几乎总是"机器不是我独占"：同机混部其他计算进程，或容器配额只给了部分核；此时人人默认"整机线程数"，过订阅带来的上下文切换与缓存失效会吃掉并行收益，按份额显式切分才能换来稳定的延迟。
