...
悬崖的根源在机制上：`map_elements` 无法把 Python 函数编译进引擎，执行时只能把整列拆成一个个 Python 标量逐个传入，函数返回后再把结果逐个装回 Series——每次往返都跨越一次 Python↔Rust 边界、经历装箱拆箱，单次开销固定、无法摊薄，总成本 = 单次开销 × 行数，随行数线性放大；原生表达式则整列留在 Rust 内核里一次向量化完成。因此适用边界很窄：确无原生表达式可用（自定义加密、调用只有 Python 绑定的库），且行数少或调用频率低；一旦要对百万行逐行执行，出路是 13.2 的 plugin，而不是优化 lambda 本身。下面的实测给出量级差距：
...
这两个主题是同一个决策的两面：何时值得付 pandas 边界的转换税，以及付不起时如何把税压到最低。转换成本并不均匀——数值列经 Arrow 缓冲区近乎零拷贝，object 列却要逐值装箱重建，同为百万行能差出一个量级（下文实测），所以"必要时"的判断标准是列的 dtype 而不是行数。迁移也不必一步到位：三步法把转换点逐步推向系统边界，每一步都可验证、可回滚，风险始终可控。
...
**第 2 步 · 瓶颈段逐个替换**：用第 12 章的分段计时（perf_counter）定位最慢的一段，改成 Polars 惰性管道（scan → 变换 → sink），其余照旧；每替换一段做一次回归对比，确认数值一致、耗时下降。
