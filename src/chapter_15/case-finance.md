...
三张表各司其职，也预示了全章的计算骨架：bars 按时间连续排列，是对齐用的"基准轨"；signals 是稀疏事件流，只在信号日有行；symbols 是低基数主数据。回测的第一步永远是"事件对齐到行情"（join_asof，即 15.2 的开场），之后才谈指标。输出端的三个指标各代表一类计算形态：日收益靠分组聚合、最大回撤靠累计路径（cum_sum/cum_max）、夏普靠滚动窗口——聚合（第 10 章）与时序（第 11 章）的工具由此在一条链上全部用一遍。三张表的列结构如下：
...
这三个口径陷阱的共性是：错了不报错。窗口口径不一致的夏普、被当日创新高污染的回撤、与窗口粒度不匹配的年化因子，输出的都是"看起来合理"的数字，只有交叉检验或样本外复测才能暴露。排查回测异常时，先查统计口径、再怀疑策略逻辑，顺序反了会在错误的问题上空耗时间。
